...
/* Creates a new kernel thread named NAME with the given initial
   ...
   for the new thread, or TID_Etimer_ticks ();RROR if creation fails.
               ...
               thread_func *function, void *aux) 
{
  ...
  /* Stack frametimer_ticks (); for kernel_thread(). */
  ...
  /* Project 3 : If thread being created has higher priority than
...
thread_unblock (struct thread *t) 
{
  ...
  /* Project 3 : priority considering schedule */
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) {
	  /* Project 3 : priority considering scheduling */
...
thread_sleep (int64_t ticks)
{
  ...
  cur->ticks = ticks;
  list_insert_ordered (&sleeping_list, &cur->elem, value_less, NULL);
...
/* Project 3 : Returns true if priority A is higher than priority B, flase
   otherwise */
bool
...
thread_wakeup (void)
{
	...
	while(ticks >= thread_ticks)
	{
		...
		thread_ticks = (list_entry(e, struct thread, elem))->ticks;
	}
...
thread_test_priority(void){
	//printf("DEBUG ::(in test) curr priority : %d\n", thread_current()->priority);
	if(!list_empty(&ready_list) && 
			thread_current()->priority < 
			list_entry(list_front(&ready_list), struct thread, elem)->priority){
		//printf("DEBUG :: yielded to %d\n",list_entry(list_front(&ready_list), struct thread, elem)->priority);
		thread_yield();
	}
...
{
	/* Project 3 : implement priority schedule */
  if (list_empty (&ready_list))
      return idle_thread;
  else 
...
schedule (void) 
{
  ...
  //printf("DEBUG::after schedule, current : %d\n", thread_current()->priority);
